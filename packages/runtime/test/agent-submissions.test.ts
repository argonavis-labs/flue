...
describe('processSubmission()', () => {
	...
	it('still settles the failed dispatch when the advisory append itself fails', async () => {
		const provider = createProvider();
		const store = await openExecutionStore();
		...
		// Mid-turn: an assistant message is in progress, so the advisory append
		// throws the reducer's linear-append invariant instead of landing.
		const timestamp = new Date().toISOString();
		await writer.append([
			{
				v: 1,
			...
			},
			{
				...
				submissionId: 'direct-1',
				attemptId: 'attempt-0',
				id: 'record_assistant_started_direct-1',
				type: 'assistant_message_started',
				messageId: 'entry_assistant_direct-1',
				parentId: INPUT_ENTRY_ID,
		...
		// Read the durable stream, not the writer: the refused advisory append
		// failed this writer instance by design.
